...
void Application::Display(void)
{
	...
	case 3:
		m_pCamera->ResetCamera();
		m_pCamera->SetPositionTargetAndUpward(AXIS_X*25.0f, AXIS_Y, -1.0f*AXIS_Z);
		break;
	case 4:
		m_pCamera->ResetCamera();
	...
	case 5:
		m_pCamera->ResetCamera();
		m_pCamera->SetPositionTargetAndUpward(AXIS_Z * -15.0f, ZERO_V3, AXIS_Y);
		m_pCamera->SetNearFar(vector2(100.0f,5.0f));
		break;
	case 6:
		m_pCamera->ResetCamera();
		m_pCamera->SetPositionTargetAndUpward(AXIS_Z *1.0f, ZERO_V3, AXIS_Y);
		m_pCamera->SetNearFar(vector2(20.0f,1.0f));
		m_pCamera->SetPerspective(false);
		break;
	case 7:
		m_pCamera->ResetCamera();
		m_pCamera->SetPositionTargetAndUpward(AXIS_Z * 10.0f, ZERO_V3, -1.0f*AXIS_Y);
		break;
	}
